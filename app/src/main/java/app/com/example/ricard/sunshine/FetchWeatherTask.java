...
import app.com.example.ricard.sunshine.data.WeatherContract.WeatherEntry;
import app.com.example.ricard.sunshine.data.WeatherDbHelper;
...
public class FetchWeatherTask extends AsyncTask<String, Void, Void> {
    ...
    /*private String formatHighLows(double high, double low) {
        ...
        String unitType = sharedPrefs.getString(
                mContext.getString(R.string.temp_units_preference_key),
                mContext.getString(R.string.temp_units_preference_default_value));

        if (unitType.equals(mContext.getString(R.string.temp_units_imperial_value))) {
            high = (high * 1.8) + 32;
            low = (low * 1.8) + 32;
        } else if (!unitType.equals(mContext.getString(R.string.temp_units_metric_value))) {
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        // Create a ContentValues var containing the input variables
        ...
        if (locationCursor.moveToFirst()) {
            return_id = locationCursor.getInt(locationCursor.getColumnIndex(WeatherContract.LocationEntry._ID));
            //Log.e(LOG_TAG,"There is already a record!");
        }else{
            ...
            return_id = (int) ContentUris.parseId(locationUri);
            Log.e(LOG_TAG,"I created a new record!");
        }
        //Log.e(LOG_TAG,"THE LOCATION SETTING IS" + values.get(WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING));
        //Log.e(LOG_TAG,"THE ID FOR THE ROW IS"+return_id);
        return return_id;
    }
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                ContentValues a1 = values[0];
                a1.get("max");
                Log.e(LOG_TAG,"EL vector que metoenla basetienevalor"+ a1);

                mContext.getContentResolver().bulkInsert(WeatherContract.WeatherEntry.CONTENT_URI,values);
                Log.e(LOG_TAG,"afegint...");
            ...
            Log.e(LOG_TAG, "FetchWeatherTask Complete. " + cVVector.size() + " Inserted");

            ContentValues a = cVVector.get(0);
            a.get(WeatherEntry.COLUMN_MAX_TEMP);
            Log.e(LOG_TAG,"max temp is "+a);
    ...
    //protected void onPostExecute(String[] result) {
        /*No need to put the data on ForecastAdapter, this is automatically managed by the loader?
        if (result != null ) {
